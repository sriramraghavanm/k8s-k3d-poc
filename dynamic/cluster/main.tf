locals {
  ...
  persistent_volume_path = var.persistent_volume_path != "" ? var.persistent_volume_path : "/tmp/k3d-${local.cluster_name}-storage"
}
...
resource "null_resource" "k3d_cluster" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      k3d cluster delete ${local.cluster_name} 2>/dev/null || true

      # Ensure persistent volume path exists on host
      mkdir -p "${local.persistent_volume_path}"
      ...
      if [ "${var.enable_registry}" = "true" ]; then
        if command -v lsof >/dev/null 2>&1; then
          if lsof -nP -iTCP:${var.registry_port} -sTCP:LISTEN >/dev/null 2>&1; then
            echo "ERROR: registry port ${var.registry_port} is already in use." >&2
            exit 1
          fi
        else
      ...
      eval k3d cluster create "${local.cluster_name}" \
        ...
        --volume "${local.persistent_volume_path}:/var/lib/rancher/k3s/storage@all" \
      ...
      kubectl --context "k3d-${local.cluster_name}" wait \
        --for=condition=Ready nodes --all --timeout=300s

      echo "Cluster '${local.cluster_name}' created successfully!"
    EOT
  }
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      k3d cluster delete ${self.triggers.cluster_name} 2>/dev/null || true
...
resource "null_resource" "verify_cluster" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      echo "Verifying cluster access for '${local.cluster_name}'..."
      kubectl config use-context "k3d-${local.cluster_name}"
